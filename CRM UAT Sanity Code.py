...
try:
    ...
    # Locate Settings
    settings_xpath = "//*[name()='path' and contains(@d,'M13.8772 1')]"
    settings_menu = wait.until(EC.visibility_of_element_located((By.XPATH, settings_xpath)))
    
    # Move to Settings and PAUSE for a second to allow the CSS transition
    actions.move_to_element(settings_menu).pause(1).perform()
    print("🖱️ Hovered over Settings")
    
    # Locate Slot Management - Ensure it's visible after hover
    slot_xpath = "//span[text()='Slot Management'] | //*[text()='Slot Management']"
    slot_management = wait.until(EC.visibility_of_element_located((By.XPATH, slot_xpath)))
    
    # Click it
    slot_management.click()
    print("✅ Slot Management clicked")

except Exception as e:
    print(f"❌ Navigation failed: {e}")
    # Fallback: If hover fails, try clicking Settings first then the sub-menu
    driver.find_element(By.XPATH, settings_xpath).click()
    time.sleep(1)
    driver.find_element(By.XPATH, slot_xpath).click()
